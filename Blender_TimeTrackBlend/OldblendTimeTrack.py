...
def update_tracking():
    ...
    try:
        ...
        last_update = tracking_data["last_update_time"]
        
        # Calculate time elapsed since last update
        time_elapsed = current_time - last_update
        
        # Only add time if not idle
        if current_time - last_activity <= tracking_data["idle_threshold"]:
            ...
            if hasattr(bpy.context.scene, "time_tracker_props"):
                ...
                props.session_time += time_elapsed
        
        # Update last update time
        tracking_data["last_update_time"] = current_time
    except:
        pass
...
def register_activity():
    ...
    # If this is the first activity, also set it as tracking start
    if not tracking_data["is_tracking"]:
...
def on_load_post(dummy):
    ...
    try:
        ...
        print(f"Time Tracker: Error in load_post - {e}")
...
def on_save_post(dummy):
    ...
    try:
        ...
        print(f"Time Tracker: Error in save_post - {e}")
...
class TIME_TRACKER_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if tracking_data["modal_running"]:
            box.label(text="  Modal: Running", icon='CHECKMARK')
        else:
            box.label(text="  Modal: Not Running", icon='ERROR')
            box.operator("time_tracker.start_modal", text="Start Tracking")
    ...
    def execute(self, context):
        if not tracking_data["modal_running"]:
            try:
                ...
                self.report({'ERROR'}, f"Failed to start tracking: {e}")
                return {'CANCELLED'}
        else:
            self.report({'INFO'}, "Tracking already running")
...
class TIME_TRACKER_OT_modal(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type in {'MOUSEMOVE', 'LEFTMOUSE', 'RIGHTMOUSE', 'MIDDLEMOUSE', 
                         'WHEELUPMOUSE', 'WHEELDOWNMOUSE', 'PRESSBUTTON', 'RET',
                         'ESC', 'TAB', 'SPACE'} or event.value == 'PRESS':
            register_activity()
    ...
    def execute(self, context):
        ...
        print("Time Tracker: Modal operator started")
        return {'RUNNING_MODAL'}
    ...
    def cancel(self, context):
        ...
        print("Time Tracker: Modal operator stopped")
...
def start_modal_delayed():
    """Delayed start for modal operator"""
    try:
        if not tracking_data["modal_running"] and bpy.data.filepath:
            bpy.ops.time_tracker.modal()
            print("Time Tracker: Started via timer")
    except Exception as e:
        print(f"Time Tracker: Failed to start - {e}")
...
def register():
    ...
    # Try to start modal after a delay, but don't fail if it doesn't work
    try:
        bpy.app.timers.register(start_modal_delayed, first_interval=2.0)
    except:
        pass
    
    print("Time Tracker: Addon registered")
...
def unregister():
    ...
    print("Time Tracker: Addon unregistered")